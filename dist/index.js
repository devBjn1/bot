...
const HelpMessage_1 = __importDefault(require("./command/HelpMessage"));
const CreateTask_1 = require("./command/CreateTask");
const PhotoCommand_1 = __importDefault(require("./command/PhotoCommand"));
const slashCommands = [
    {
        command: "/help",
        description: "See usage instructions",
        handler: HelpMessage_1.default,
    },
    {
        command: "/start",
        description: "See usage instructions",
    ...
    },
    {
        ...
        handler: CreateTask_1.createTask,
    }
];
console.log("🤖 Bot is running... Waiting for images to create tasks.");
console.log("📊 Tasks will be sent to group:", config_1.BUG_LOG_GROUP_ID);
config_1.telegram.setBotCommands(slashCommands.map(({ command, description }) => ({
    command: command.replace("/", ""),
    description,
})));
slashCommands.forEach(({ command, handler }) => {
    config_1.telegram.onCommand(command, handler);
});
...
// }
// Photo handler
config_1.telegram.onPhoto(PhotoCommand_1.default);
// telegram.onPhoto(async (msg: any) => {
//   const userId = msg.from.id;
//   const photos = msg.photo || [];
//   const best = photos[photos.length - 1];
//   const text = msg.text as string;
//   if (!text.trim().startsWith("/create")) {
