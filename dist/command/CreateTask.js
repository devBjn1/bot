...
async function createTaskService(content, chatId, userId, photos) {
    ...
    try {
        const card = await config_2.taskService.createCard(title, description);
        console.log("Trello card created:", card);
        // store pending task with cardId so images sent afterwards are attached to this card
        config_2.taskManager.addPendingTask(userId, title, description, card.id);
        if (photos.length > 0) {
            await uploadListPhotoToTask(chatId, card, photos);
        }
        else {
            await config_1.telegram.sendMessage(chatId, `✅ Task created! View on Trello: ${card.shortUrl || card.url}\n\n📸 If you want to attach images to this task, send them now and they'll be added to the card.`, { parse_mode: "Markdown" });
        }
    }
    catch (err) {
        console.error("Error creating Trello card:", err?.message || err);
        // fallback: create a pending template without a card
        config_2.taskManager.addPendingTask(userId, title, description);
        await config_1.telegram.sendMessage(chatId, `✅ Task template created! (Trello create failed)\n\n*Title:* ${title}\n*Description:* ${description}\n\n📸 You can send images later and I'll try to attach them when Trello is reachable.`, { parse_mode: "Markdown" });
    }
}
async function uploadListPhotoToTask(chatId, task, photos) {
    for (let idx = 0; idx < photos.length; idx++) {
        ...
        try {
            const fileUrl = await config_1.telegram.getFileUrl(p.file_id);
            try {
                await config_2.taskService.addAttachmentFromUrl(task.id, fileUrl, p.file_id, name);
            }
            catch (e) {
                console.warn("Multipart upload failed, falling back to URL attach:", e);
                await config_2.taskService.addAttachment(task.id, fileUrl, name);
            }
        }
        catch (attachErr) {
            console.warn("Could not attach image to Trello card:", attachErr?.message || attachErr);
        }
    }
    await config_1.telegram.sendMessage(chatId, `✅ Task created! View on Trello: ${task.shortUrl || task.url}\n\n📸 If you want to attach images to this task, send them now and they'll be added to the card.`, { parse_mode: "Markdown" });
