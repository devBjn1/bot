  ...
  try {
    ...
    if (captionCreate) {
      ...
      for (let idx = 0; idx < photos.length; idx++) {
        ...
        try {
          ...
          try {
            await trello.addAttachmentFromUrl(
              card.id,
              fileUrl,
              p.file_id,
              name
            ...
            await trello.addAttachment(card.id, fileUrl, name);
          }
        } catch (attachErr: any) {
          console.warn(
    ...
    for (let idx = 0; idx < photos.length; idx++) {
      ...
      try {
        const fileUrl = await telegram.getFileUrl(p.file_id);
        try {
          await trello.addAttachmentFromUrl(card.id, fileUrl, p.file_id, name);
        } catch (e) {
          console.warn(
            "Multipart upload failed, falling back to URL attach:",
            e
...
telegram.onText(async (msg: any) => {
  ...
  if (text === "/start" || text === "/help") {
    await telegram.sendMessage(
      chatId,
      "🤖 *Task Logging Bot*\n\n*How to create a task:*\n\n*Please use `/create {title}. {description}` first, then send the image(s) to complete the task.\n\n*Format:*\n`/create {title}. {description}`\n\n*Example:*\n`/create Bug on login page. Cannot click submit button after entering credentials`",
      { parse_mode: "Markdown" }
    );
    return;
  }
